...
public class MockTreeifyService {
    ...
    public GenerateTaskDto createGenerateTask(Long projectId, CreateGenerateTaskRequest request) {
        ...
        requireText(request == null ? null : request.input(), "需求输入不能为空");
    ...
    public List<GenerateSseEventDto> buildMockGenerateEvents(String taskId) {
        getTask(taskId);
        List<GeneratedCaseDto> generatedCases = mockGeneratedCases();
        return List.of(
                event(taskId, STAGE_STARTED, "e1", 1, Map.of("stage", "e1")),
                event(taskId, STAGE_CHUNK, "e1", 2, Map.of("content", "正在解析需求目标、用户动作和系统约束...")),
                ...
                event(taskId, STAGE_CHUNK, "e2", 5, Map.of("content", "正在拆分可测试对象：登录表单、账号校验、错误提示、跳转流程...")),
                ...
                event(taskId, STAGE_CHUNK, "e3", 8, Map.of("content", "正在生成可执行测试用例，并补齐前置条件、步骤和预期结果...")),
    ...
    private List<GeneratedCaseDto> mockGeneratedCases() {
        return List.of(
