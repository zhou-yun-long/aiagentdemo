...
export function useGenerateStream() {
  ...
  const startRealGeneration = useCallback(
    async (input: string, mode: GenerationMode) => {
      ...
      const eventSource = new EventSource(getGenerateStreamUrl(task));
    ...
    [applyRealEvent, clearTimers, closeEventSource]
  );
  ...
  const confirmCurrentStage = useCallback(async () => {
    ...
    if (source === 'real' && taskId) {
      await confirmGenerateTask(taskId, { stage: activeStage });
